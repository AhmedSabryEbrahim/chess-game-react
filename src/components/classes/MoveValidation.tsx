...
export class MoveValidator {
  ...
  public isValidVerticalMove(
    ...
    if (
      MoveValidator.validPoint(source) ||
      MoveValidator.validPoint(destination)
    )
      return false;
  ...
  public isValidDiagonalMove(
    ...
    else if (Math.abs(distance.x) > max || Math.abs(distance.y) > max)
      return false;
    return true;
  }
  public isValidNightMove(source: Point, destination: Point): boolean {
    ...
    else if (Math.abs(distance.x) === 2 && Math.abs(distance.y) === 1)
      return true;
    return false;
  }
}
